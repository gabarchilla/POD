document.addEventListener('DOMContentLoaded', () => {
    ...
    const previewTbody = document.getElementById('preview-novedades');
    ...
    saveBtn.addEventListener('click', () => {
        const parque_id = document.getElementById('parque-novedades').value;
        const hora = document.getElementById('hora').value;
        const codigo = document.getElementById('codigo').value;
        const descripcion = document.getElementById('descripcion').value;

        if (!parque_id || !hora || !codigo || !descripcion) {
            alert('Completa todos los campos');
            return;
        }

        const record = { parque_id, hora, codigo, descripcion };
        records.push(record);
        ...
        document.getElementById('novedades-form').reset();
    });
    ...
    sendBtn.addEventListener('click', () => {
        ...
        fetch('/api/confirm_save', {
            ...
            body: JSON.stringify({
                novedades_operativas: records.map(r => ({ ...r, fecha_id: fechaId }))
            })
        })
